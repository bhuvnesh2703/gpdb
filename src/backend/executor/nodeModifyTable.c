...
ExecModifyTable(PlanState *pstate)
{
	...
	for (;;)
	{
		...
		if (operation == CMD_UPDATE || operation == CMD_DELETE)
		{
			...
			{
				/* Only foreign tables are allowed to omit a row-ID attr */		Assert(relkind == RELKIND_FOREIGN_TABLE);
...
ExecInitModifyTable(ModifyTable *node, EState *estate, int eflags)
{
	...
	{
		mtstate->mt_isSplitUpdates = node->isSplitUpdates;
	...
	for (i = 0; i < nrels; i++)
	{
		...
		if (operation == CMD_UPDATE || operation == CMD_DELETE)
		{
				...
				relkind == RELKIND_PARTITIONED_TABLE)
			{
				...
				resultRelInfo->ri_segid_attno = ExecFindJunkAttributeInTlist(subplan->targetlist, "gp_segment_id");
				if (!AttributeNumberIsValid(resultRelInfo->ri_segid_attno))
					elog(ERROR, "could not find junk gp_segment_id column");

				if (operation == CMD_UPDATE && mtstate->mt_isSplitUpdates) {
					resultRelInfo->ri_action_attno = ExecFindJunkAttributeInTlist(subplan->targetlist, "DMLAction");
					if (!AttributeNumberIsValid(resultRelInfo->ri_action_attno))
			...
			else if (relkind == RELKIND_FOREIGN_TABLE)
			{
				 ...
				 * a wholerow attribute.  We also require it to be present in
